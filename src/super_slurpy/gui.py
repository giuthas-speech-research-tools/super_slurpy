...
class SnakeGUI(QMainWindow):
    ...
    def _init_ui(self) -> None:
        ...
        self.btn_open = QPushButton(text="Open Video (Ctrl+O)", parent=main_widget)
        self.btn_open.clicked.connect(slot=self.open_video)

        self.btn_track = QPushButton(text="Track (Ctrl+T)", parent=main_widget)
        self.btn_track.clicked.connect(slot=self.run_tracking)
        self.btn_track.setEnabled(False)

        self.btn_save = QPushButton(text="Save CSV (Ctrl+S)", parent=main_widget)
        self.btn_save.clicked.connect(slot=self.save_results_to_csv)

        self.btn_load = QPushButton(text="Load CSV (Ctrl+L)", parent=main_widget)
        ...
        self.canvas.mpl_connect(s="button_press_event", func=self.on_mouse_press)
        self.canvas.mpl_connect(s="motion_notify_event", func=self.on_mouse_motion)
        self.canvas.mpl_connect(s="button_release_event", func=self.on_mouse_release)
    ...
    def _init_menus(self) -> None:
        ...
        action_load_seed_spline = QAction(text="&Load seed spline...", parent=self)
        action_load_seed_spline.triggered.connect(slot=self.load_seed_spline)
        file_menu.addAction(action_load_seed_spline)

        action_save_spline = QAction(text="&Save current spline as default", parent=self)
        ...
        self.action_resample = QAction(text="&Resample Splines...", parent=self)
        ...
        self.action_apply_seed = QAction(text="&Apply seed spline", parent=self)
        self.action_apply_seed.triggered.connect(slot=self.apply_seed_spline)
        self.action_apply_seed.setEnabled(self.model.seed_spline is not None)
        action_menu.addAction(self.action_apply_seed)

        self.action_clear_splines = QAction(text="&Clear All Splines", parent=self)
        self.action_clear_splines.triggered.connect(slot=self.clear_all_splines)
    ...
    def open_video(self) -> None:
        file_path, _ = QFileDialog.getOpenFileName(
            parent=self, caption="Open video", directory="", filter=VIDEO_FILTER
        )
        if not file_path:
            return
    ...
    def _display_canvas(self) -> None:
        ...
        self.ax.imshow(X=self.model.frame, aspect="equal", extent=[0, width, height, 0])
    ...
    def on_mouse_press(self, event: Any) -> None:
        ...
        closest_idx: int | None = self._get_closest_anchor(x=event.xdata, y=event.ydata)
    ...
    def save_results_to_csv(self) -> None:
        if not self.model.anchors_history:
            self.statusBar().showMessage("Warning: No tracking data to save.", 5000)
            return
        ...
        self.statusBar().showMessage(f"Success: Results saved to {file_path}", 5000)
    ...
    def load_results_from_csv(self) -> None:
        ...
        try:
            ...
            if self.model.current_frame_idx in self.model.anchors_history:
                pts = self.model.anchors_history[self.model.current_frame_idx]
                self.model.anchors = [list(pt) for pt in pts]
                self.model.update_spline()
                self._display_canvas()

            self.statusBar().showMessage("Success: Results loaded successfully.", 5000)

        except Exception as e:
            self.statusBar().showMessage(f"Error: Failed to load CSV: {e}", 5000)
    ...
    def run_tracking(self) -> None:
        ...
        # Custom logic here to update GUI mid-tracking
    ...
    def resample_splines(self) -> None:
        ...
        for frame_idx, pts_list in self.model.anchors_history.items():
            ...
            k_new: np.ndarray = np.linspace(start=0, stop=k[-1], num=num_points)
        ...
        if self.model.current_frame_idx in self.model.anchors_history:
            self.model.anchors = [
                list(pt) for pt in self.model.anchors_history[self.model.current_frame_idx]
            ]
    ...
    def save_seed_spline(self) -> None:
        ...
        try:
            ...
            self.statusBar().showMessage(f"Success: Spline saved to {file_path}", 5000)
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Failed to save spline: {e}")
    ...
    def load_seed_spline(self) -> None:
        ...
        try:
            ...
            if self.model.seed_spline:
                ...
                self.model.update_spline()
                self._display_canvas()
                self.statusBar().showMessage("Success: Seed spline loaded.", 5000)

        except Exception as e:
            QMessageBox.critical(
                self, "Format Error", f"Failed to load spline correctly.\n\n{e}"
            )
...
def launch_gui(video_path: str | None = None, seed_path: str | None = None) -> None:
