...
Process video files without launching the GUI using the `track`
command. You can process a single video or an entire directory
of videos (.mp4, .avi, .mkv) sequentially.

```bash
slurpy track [INPUT_PATH] [OPTIONS]
```
...
* `-o, --output`: Output CSV path (single file) or directory.
  Defaults to the input path with a `_tracked.csv` suffix.
* `-m, --method`: The tracking algorithm to execute. Choices
  are `snake` or `particle`. Defaults to `snake`.
